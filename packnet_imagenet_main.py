...
def main():
    ...
    if args.arch == 'resnet50' or 'resnet18':
    ...
    # crop means the original image is not 224*224
    if 'cropped' in args.dataset:
        train_loader = dataset.train_loader_cropped(args.path, args.dataset, args.batch_size)
        val_loader = dataset.val_loader_cropped(args.path, args.dataset, args.val_batch_size)
    else:
        train_loader = dataset.train_loader(args.path, args.dataset, args.batch_size)
        val_loader = dataset.val_loader(args.path, args.dataset, args.val_batch_size)
    ...
    if args.mode == 'inference':
        ...
        avg_val_acc = manager.validate(resume_from_epoch-1)
        if args.logfile:

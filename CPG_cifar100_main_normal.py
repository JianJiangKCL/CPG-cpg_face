...
def main():
    ...
    predefined_acc = 0.85
    logging.info(f"Now it's dataset {args.dataset} in mode {args.mode}")
    ...
    if args.num_classes == 2:
        train_loader = dataset.cifar100_train_loader_two_class(args.path, args.dataset, args.batch_size)
        val_loader = dataset.cifar100_val_loader_two_class(args.path, args.dataset, args.val_batch_size)
    ...
    if avg_train_acc >predefined_acc:
        manager.save_checkpoint(optimizers, epoch_idx, args.save_folder)
    else:
        # pass

        #todo this is added by me, seems without it no ckpt is saved
        manager.save_checkpoint(optimizers, epoch_idx, args.save_folder)
